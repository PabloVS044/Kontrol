    ...
    <div class="pd-layout">
      ...
      <template v-else-if="project">
          ...
          <div v-else class="tasks-list">
            ...
            >
              <div class="task-card-left">
                <span
                  class="priority-bar"
                  :style="{ background: priorityColor(task.prioridad) }"
                ></span>
                <div class="task-info">
                  <span class="task-name">{{ task.nombre }}</span>
                  <span v-if="task.descripcion" class="task-desc">{{
                    task.descripcion
                  }}</span>
                  <div class="task-badges">
                    <span
                      class="status-badge small"
                      :style="statusStyle(task.estado)"
                    >
              ...
              <div v-if="canManageTasks" class="task-card-actions">
                <button class="task-action-btn" @click="openEditTask(task)">
                  Edit
                </button>
                <button
                  v-if="
        ...
        <section v-if="activeTab === 'team'" class="tab-panel">
          <p class="team-note">
            Role breakdown for this project. To manage individual members and
            permissions, use the
            <RouterLink to="/dashboard" class="team-note-link"
...
function priorityColor(p) {
  return (
    { BAJA: "#555", MEDIA: "#60a5fa", ALTA: "#f97316", CRITICA: "#fb7185" }[
      p
    ] || "#555"
  );
}
