    ...
    <div v-if="model" class="modal-overlay" @click.self="model = false">
      <div class="modal">
        <div class="modal-header">
          <span class="modal-title">{{ editing ? 'Edit activity' : 'New activity' }}</span>
        ...
        <form class="modal-form" @submit.prevent="$emit('submit')">
          ...
          <div class="form-row">
            <div class="form-field">
              ...
              <input v-model.number="form.monto_planificado" type="number" min="0" step="0.01" required />
            </div>
            <div class="form-field">
              <label>Actual</label>
              <input v-model.number="form.monto_real" type="number" min="0" step="0.01" />
            </div>
          </div>
