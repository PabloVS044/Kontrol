...
| Caso | Historia/Ticket | Descripción | Elemento bajo prueba | Test automatizado | Estado |
|---|---|---|---|---|---|
| C1 | HU-38 / SCRUM-28 | Carga: inicio de sesión concurrente dentro del umbral de latencia y error | `POST /api/auth/login` | `k6/load-test.js` | Cubierto: corrió, umbral de latencia no cumplido (p95=4.99s vs 300ms), 0% error — ver `docs/pruebas-carga-estres.md` §2 |
| C2 | HU-38 / SCRUM-28 | Carga: listado de proyectos y métricas dentro del umbral de latencia y error | `GET /api/projects`, `GET /api/projects/:id/metrics` | `k6/load-test.js` | Cubierto: corrió, umbral de latencia no cumplido (p95=1.91s vs 500ms), 0% error |
...
### 19.6 Casos del centro de marketing y bordes de exportación, SCRUM-35

Los casos 1 a 6 verifican el centro de marketing de SCRUM-19; los casos 7 y 8 refuerzan la exportación a CSV de SCRUM-20 en dos bordes que §19.1 no cubría: el conjunto vacío y el repertorio completo de caracteres del español. Las suites S2, S3 y S4 de §19.3 ya respaldaban los casos 1 a 6 como regresión; esta sección les da caso, descripción y estado propios, como exige el criterio de aceptación de SCRUM-35.

| Caso | Historia/Ticket | Descripción | Elemento bajo prueba | Test automatizado | Estado |
|---|---|---|---|---|---|
| M1 | SCRUM-35 / SCRUM-19 | Creación de publicación con campos válidos: los catorce campos se persisten íntegros y la respuesta llega mapeada | `backend/src/controllers/marketingController.js` → `createMarketingPublication` | `backend/tests/marketingPublications.test.js` → Caso 6 · Creación con campos válidos | Cubierto |
| M2 | SCRUM-35 / SCRUM-19 | Transición de estado de borrador a programada | `backend/src/utils/marketingPublicationLifecycle.js` → `isPublicationTransitionAllowed` | `backend/tests/marketingPublicationLifecycle.test.js` → Caso 2 · Transiciones permitidas | Cubierto |
| M3 | SCRUM-35 / SCRUM-19 | Transición de estado de programada a publicada | `backend/src/utils/marketingPublicationLifecycle.js` → `isPublicationTransitionAllowed` | `backend/tests/marketingPublicationLifecycle.test.js` → Caso 2 · Transiciones permitidas | Cubierto |
| M4 | SCRUM-35 / SCRUM-19 | Rechazo de transición inválida: publicada es terminal, responde 409 y no escribe en la base | `marketingPublicationLifecycle.js` → `resolvePublicationLifecycleForUpdate`; `marketingController.js` → `updateMarketingPublication` | `backend/tests/marketingPublicationLifecycle.test.js` → Caso 3 · Publicada es un estado terminal; `backend/tests/marketingPublications.test.js` → Caso 4 · Las transiciones inválidas no llegan a escribir | Cubierto |
| M5 | SCRUM-35 / SCRUM-19 | Filtrado del listado por estado, canal y proyecto, con el canal como enum `platform` | `marketingController.js` → `getMarketingPublications`; `frontend/src/views/MarketingPublicationsView.vue` | `backend/tests/marketingPublications.test.js` → Caso 3 · Listado filtrable por estado, canal y proyecto; `frontend/tests/marketingPublicationsView.test.js` → Filtros por estado, canal y proyecto | Cubierto |
| M6 | SCRUM-35 / SCRUM-19 | Aislamiento multi-empresa: una empresa no accede a publicaciones de otra, ni al listar, ni al leer, ni al borrar | `marketingController.js`, filtro `id_empresa` en toda consulta; `requireCompany` | `backend/tests/marketingPublications.test.js` → Caso 2 · Aislamiento multi-empresa | Cubierto |
| M7 | SCRUM-35 / SCRUM-20 | Exportación con conjunto de datos vacío: genera archivo válido con cabecera, BOM y CRLF, sin lanzar error | `frontend/src/utils/csvExport.js` → `toCsv`, `csvBlob`; `reportExport.js` → `renderExport` | `frontend/tests/csvExportEdgeCases.test.js` → Caso 7 · Conjunto de datos vacío | Cubierto |
| M8 | SCRUM-35 / SCRUM-20 | Exportación con caracteres especiales y acentos: vocales acentuadas, diéresis, eñe, comillas, emojis y símbolos de moneda se codifican correctamente | `frontend/src/utils/csvExport.js` → `escapeCsvValue`, `guardFormula`, `toCsv` | `frontend/tests/csvExportEdgeCases.test.js` → Caso 8 · Caracteres especiales y acentos | Cubierto |

### 19.7 Casos del contrato de parámetro de ruta en producto-proveedor, SCRUM-41

Los seis casos fijan el contrato del parámetro de ruta de los tres endpoints que relacionan un producto con sus proveedores, corregidos en SCRUM-41 a partir del elemento DT-04 del inventario de deuda técnica. Las aserciones se hacen sobre los parámetros con los que se arma la consulta y no solo sobre el código de estado: el defecto corregido hacía que el identificador del proveedor llegara vacío a la capa de datos, de modo que la consulta se ejecutaba sin error, no encontraba fila y respondía un 404 indistinguible del de una relación que de verdad no existe. El caso PP5 aísla esa distinción, y es la razón por la que la suite mira parámetros.

| Caso | Historia/Ticket | Descripción | Elemento bajo prueba | Test automatizado | Estado |
|---|---|---|---|---|---|
| PP1 | SCRUM-41 / DT-04 | La actualización de un enlace arma el UPDATE con el identificador de proveedor que viene en la URL | `backend/src/controllers/productController.js` → `updateSupplierLink`; `productRoutes.js` → `PUT /:id/suppliers/:supplierId` | `backend/tests/productSuppliers.controller.test.js` → T1 | Cubierto |
| PP2 | SCRUM-41 / DT-04 | La eliminación de un enlace arma el DELETE con el identificador de proveedor que viene en la URL | `productController.js` → `unlinkSupplier`; `productRoutes.js` → `DELETE /:id/suppliers/:supplierId` | `backend/tests/productSuppliers.controller.test.js` → T2 | Cubierto |
| PP3 | SCRUM-41 / DT-04 | El enlace de un proveedor inserta con el producto de la URL y el proveedor del cuerpo | `productController.js` → `linkSupplier`; `productRoutes.js` → `POST /:id/suppliers` | `backend/tests/productSuppliers.controller.test.js` → T3 | Cubierto |
