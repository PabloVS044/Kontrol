...
|---|---|---|---|---|---|---|---|---|
| DT-01 · **resuelto 24/09/2026** | Backend / robustez | No existe middleware de errores ni envoltorio `asyncHandler`. 19 `throw err` en 8 controladores viajan como *promise rejection* no capturada; Express 4 no las intercepta y Node 20+ termina el proceso | El proyecto creció endpoint a endpoint sin definir nunca la capa de errores transversal | Una violación de FK o un fallo de `pool` en cualquier endpoint tumba el backend entero para todos los usuarios | 3 | **P1** | Extracción de método (`asyncHandler`) + middleware central de errores, con pruebas de caracterización del formato de error actual | 8 |
...
**Estado: resuelto el 24/09/2026.** `backend/src/middleware/asyncHandler.js` define `asyncHandler` y `createRouter()`, que sustituye a `Router()` en los 17 archivos de rutas y envuelve cada handler y middleware al registrarlo: las 19 sentencias quedan cubiertas, y también los casos que el inventario original no contaba —`pool.connect()` fuera del `try` en varios controladores y los middlewares async `requireCompany`, `requireProject` y `requireProjectPermission`, que no tenían `catch`—. `backend/src/middleware/errorHandler.js` va al final de la cadena en `index.js` y mantiene el contrato `{ success: false, message }`: 409 para violaciones de FK (23503) o unicidad (23505) no manejadas, el status propio para errores 4xx y 500 genérico para el resto, sin exponer el detalle. `backend/tests/errorContract.test.js` caracteriza el formato de error previo (T-01) y `backend/tests/errorHandler.test.js` fuerza una violación de FK, comprueba la respuesta controlada, el `ROLLBACK` y que el proceso atiende la siguiente petición sin ningún `unhandledRejection` (T-02).
...
**Estado: resuelto el 20/09/2026.** `coverage.include` está declarado en los dos workspaces, la línea base real está medida y los umbrales recalibrados sobre ella. La política actualizada, las cifras antes/después y la cobertura real por módulo están en el [`README.md`](../README.md#cobertura-de-código), sección «Cobertura de código». Resumen: backend pasa de 37.05 % sobre 1 719 sentencias a **16.21 % sobre 3 929**; frontend, de 92.90 % sobre 1 000 sentencias a **10.15 % sobre 9 152**. El numerador no se movió —637 y 929 sentencias cubiertas, las mismas— porque no se añadió ni se quitó ninguna prueba: lo que cambió es el denominador. Los tres umbrales que casaban con cero archivos se redirigieron a los archivos de reportes que sí tienen pruebas. Con esto quedan desbloqueadas T-03, T-05 y T-09, que dependían de T-11.

La evidencia original del Sprint 7, que motivó el elemento:
...
1. **T-11 va primero.** Mientras el denominador de cobertura no incluya los archivos que se van a tocar (DT-06), ninguna prueba de caracterización protege nada: el gate seguiría en verde aunque se borrara. *Hecha el 20/09/2026 junto con T-12; T-03, T-05 y T-09 quedan desbloqueadas.*
...
| T-11 · **hecha** | Declarar `coverage.include` y publicar la línea base real | DT-06 | Ajuste de configuración | `coverage.include` en ambos workspaces y línea base real medida y anotada en el README | 3 | P2 | — |
| T-12 · **hecha** | Recalibrar el trinquete y eliminar los umbrales vacíos | DT-06 | Ajuste de configuración | Umbrales globales sobre la base real y los tres globs del módulo de reportes apuntando a archivos que existen en el reporte | 2 | P2 | T-11 |
