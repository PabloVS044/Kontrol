...
**Re-corrida real de C1 (50 VUs, mismas condiciones) con rounds=6:
p95=4.00s, umbral `p(95)<300ms` sigue roto — prácticamente idéntico al
resultado original con rounds=10 (p95=4.02s reportado en §2/§3).** El costo
de bcrypt no era el cuello de botella real de C1/E1; la causa raíz del
hallazgo 1 queda re-abierta. Se intentó aislar la causa midiendo conexiones
activas en Supabase (`pg_stat_activity`) durante la corrida — inconclusa,
porque el pooler de Supabase (Supavisor, `aws-1-us-east-2.pooler.supabase.com`)
mantiene su propio pool de conexiones backend independiente del pool de la
app (`pg.Pool` en `backend/src/db/pool.js`, sin `max` fijado — default 10,
proceso único, sin cluster confirmado en código), así que contar conexiones
desde afuera no aísla la cola real de la app. Diagnóstico correcto pendiente:
instrumentar `pool.totalCount`/`pool.waitingCount` dentro de la app
(logging temporal) en vez de medir desde afuera.

| # | Hallazgo | Causa raíz | Acción de mitigación propuesta |
|---|---|---|---|
| 1 | C1, C2, C4-listado y C5 superan su umbral de p95 bajo carga sostenida; E1, E2, E4 y E5 degradan en estrés a un nivel de concurrencia bajo (40-125 VUs) — todo con 0% de errores | Se atribuía a CPU insuficiente (2 vCPU) para el costo de `bcrypt.compare` + consultas con agregación/joins bajo concurrencia. **Refutado para C1/E1**: bajar `SALT_ROUNDS` de 10 a 6 (~14x menos costo de bcrypt medido localmente) no cambió el p95 de C1 en nada (4.02s → 4.00s, re-corrida real). El costo de bcrypt no era el cuello de botella — causa raíz real aún sin confirmar (candidatos: `pg.Pool` sin `max` fijado en `backend/src/db/pool.js`, latencia de red hacia Supabase, u otra cosa) | `SALT_ROUNDS` bajo en test — implementado y desplegado, pero **no resolvió el problema**, se mantiene solo porque no hace daño. Cachear agregaciones (acción 3, no implementada) sigue pendiente para C2/C4/C5. Para C1/E1 específicamente, próximo paso real es instrumentar `pool.totalCount`/`waitingCount` en la app para confirmar si es contención de conexiones a la BD, no bcrypt. Si se reconsidera el presupuesto, subir a 4 vCPU sigue siendo mitigación directa aunque no ataque la causa raíz si termina siendo contención de pool/red |
...
| 5 | E1 y E5 degradan en su propio nivel base, sin necesitar ningún escalón de estrés adicional — el sistema ya estaba en su límite bajo carga normal, no solo bajo estrés | Se atribuía al mismo cuello de botella de CPU que el hallazgo 1 (bcrypt en E1; búsqueda+transacción con `FOR UPDATE` en E5). Para E1, refutado igual que en el hallazgo 1 — bajar `SALT_ROUNDS` no cambió el p95 de C1, así que tampoco explica la degradación de E1 | Prioridad más alta de los dos: son los dos endpoints donde un uso real y no simulado (login al iniciar el día, ventas en punto de venta) ya rozaría el límite actual. E1 y E5 siguen ambos sin mitigación efectiva — E1 necesita el mismo diagnóstico pendiente del hallazgo 1 antes de intentar otra mitigación |
